...
export function MapCanvas({
  ...
  useEffect(() => {
    ...
    try {
      ...
      const handleMapLoad = () => {
        try {
          // 1. DYNAMIC VISUALIZER GEOJSON LAYERING (Modal Swarm Agent 1 Output)
          // Polygons (BBMP Wards, Lakes, Wetlands, Slums) are strictly populated from the Visualizer Agent
          m.addSource('visualizer-features-source', {
            type: 'geojson',
            data: visualizerGeoJSON || { type: 'FeatureCollection', features: [] },
          });
          ...
          // 3. DYNAMIC CATCHMENT BUFFER (Turf.js Radial Buffer)
          ...
          // 4. Visualizer Linear Features (Rajakaluves, streams, feeder corridors)
          m.addLayer({
            id: 'visualizer-lines',
            type: 'line',
            source: 'visualizer-features-source',
          ...
          // 9. Visualizer Points (Anchor Hubs, Transit Interchanges)
          m.addLayer({
            id: 'visualizer-points',
            type: 'circle',
            source: 'visualizer-features-source',
          ...
          m.on('click', 'visualizer-points', handleVisualizerClick);
          ...
          m.on('mouseenter', 'visualizer-points', () => {
            m.getCanvas().style.cursor = 'pointer';
          });
          m.on('mouseleave', 'visualizer-points', () => {
            m.getCanvas().style.cursor = '';
          });
  ...
  // Update Dynamic Visualizer GeoJSON Layering
  useEffect(() => {
    const m = mapRef.current;
    ...
    // POIs are populated solely based on the structured output given by the visualizer agent
    const poiSource = m.getSource('pois') as maplibregl.GeoJSONSource;
    if (poiSource) {
      if (!visualizerGeoJSON || !visualizerGeoJSON.features || visualizerGeoJSON.features.length === 0) {
        ...
        // Extract Point features from visualizer output
        const allPointFeatures = visualizerGeoJSON.features.filter((f: any) => {
          const gType = f.geometry?.type;
          return gType === 'Point' || gType === 'MultiPoint';
        });

        // Filter by active categories if specified
        const filteredPointFeatures = allPointFeatures.filter((f: any) => {
          if (!activePOIFilters || activePOIFilters.length === 0) return true;
          const cat = (f.properties?.category || f.properties?.type || '').toLowerCase();
          return activePOIFilters.some((filterKey) => cat.includes(filterKey.toLowerCase()));
        });
        ...
        if (onBufferStatsChangeRef.current) {
          ...
          allPointFeatures.forEach((f: any) => {
            const raw = (f.properties?.category || f.properties?.type || f.properties?.poi_type || '').toLowerCase();
            if (raw.includes('hosp') || raw.includes('health') || raw.includes('clinic')) {
              catCounts.hospital++;
            } else if (raw.includes('edu') || raw.includes('school') || raw.includes('college') || raw.includes('univ')) {
              catCounts.education++;
            } else if (raw.includes('civic') || raw.includes('gov') || raw.includes('public')) {
              catCounts.civic++;
            } else {
              catCounts.corporate++;
            }
  ...
  useEffect(() => {
    ...
    ['visualizer-polygons-fill', 'visualizer-polygons-line'].forEach((layerId) => {
