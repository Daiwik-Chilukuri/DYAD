  ...
  return (
    <section className="w-full max-w-6xl mx-auto px-4 pt-4 pb-16">
        ...
        <div className="inline-flex items-center p-1.5 rounded-full bg-[#09090b] border border-white/[0.08] shadow-[0_20px_45px_rgba(0,0,0,0.95)] ring-1 ring-white/5 select-none">
          {CITIES_TABS.map((city) => {
            ...
            return (
              ...
              >
                {/* ACTIVE PILL GLIDE: ICONIC DYAD TURQUOISE GRADIENT */}
                {isActive && (
                  <motion.div
                    layoutId="above-flashcard-active-pill"
                    className="absolute inset-0 rounded-full bg-gradient-to-r from-[#0ab1ba] via-[#00f5d4] to-[#0ab1ba] shadow-lg shadow-[#0ab1ba]/30"
      ...
      <div className="relative w-full h-[540px] sm:h-[620px] rounded-[32px] overflow-hidden bg-[#030305] border border-white/[0.08] shadow-[0_40px_100px_rgba(0,0,0,1),0_15px_40px_rgba(0,0,0,0.9),inset_0_1px_0_rgba(255,255,255,0.08)] select-none">
        ...
        {/* BOTTOM-RIGHT DASHBOARD BUTTON (UNLINKED, ONLY RESPONDS TO CITY SELECTION) */}
        <div className="absolute bottom-5 right-6 z-20 pointer-events-auto">
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            transition={motionSprings.snappy}
            className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-white hover:bg-zinc-200 text-black font-extrabold text-xs shadow-[0_15px_35px_rgba(0,0,0,0.9),0_0_20px_rgba(255,255,255,0.12)] border border-white/80 transition-all cursor-pointer select-none"
          >
            <span>Open {currentCity.name} Dashboard</span>
